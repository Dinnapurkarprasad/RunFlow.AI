...
export default function InputNode({ id, data }) {
  ...
  const handleRun = async () => {
    ...
    setNodes(nds => nds.map(n =>
      n.id === 'output' ? { ...n, data: { ...n.data, result: 'Loading...' } } : n
    ));

    try {
      const res = await axios.post('http://localhost:5000/api/ask-ai', { prompt });
      setNodes(nds => nds.map(n => {
        if (n.id === 'output') return { ...n, data: { ...n.data, result: res.data.response } };
        if (n.id === id)       return { ...n, data: { ...n.data, currentPrompt: prompt } };
        return n;
      }));
    } catch {
      setNodes(nds => nds.map(n =>
        n.id === 'output' ? { ...n, data: { ...n.data, result: 'Error fetching response' } } : n
      ));
    }
    setLoading(false);
  };
  ...
  const nodeShadow  = isDark ? 'shadow-[0_1px_3px_rgba(0,0,0,0.4),0_4px_24px_rgba(0,0,0,0.4)]' : 'shadow-[0_1px_3px_rgba(0,0,0,0.06),0_4px_16px_rgba(0,0,0,0.06)]';
  ...
  const badgeCls    = isDark ? 'bg-[#1e2040] text-[#818cf8] border-[#252b38]' : 'bg-[#eeeeff] text-[#5b5ef4] border-[#e2e5ea]';
  const textareaBg  = isDark ? 'bg-[#1c2130] border-[#252b38] text-[#eef0f4]' : 'bg-[#f0f2f5] border-[#e2e5ea] text-[#0d0f12]';
  const phColor     = isDark ? 'placeholder:text-[#4d5668]' : 'placeholder:text-[#9ba3ae]';
  const disabledBtn = isDark ? 'bg-[#222738] text-[#4d5668]' : 'bg-[#e8eaed] text-[#9ba3ae]';

  return (
    <div className={`w-72 rounded-xl overflow-hidden border ${nodeBg} ${nodeBorder} ${nodeShadow}`}>
      <div className={`flex items-center gap-2 px-3 py-[9px] border-b ${headerBg} ${headerBdr}`}>
        ...
        <span className={`text-[11px] font-bold uppercase tracking-[0.06em] ${nodeTitle}`}>
          Input Node
        </span>
        <span className={`ml-auto text-[10px] font-semibold px-[7px] py-[2px] rounded border ${badgeCls}`}>
          Prompt
        </span>
      </div>

      <div className="p-3 pb-[14px]">
        <label className={`block text-[11px] font-semibold uppercase tracking-[0.05em] mb-1.5 ${labelColor}`}>
          Your prompt
        </label>
        <textarea
          rows={4}
